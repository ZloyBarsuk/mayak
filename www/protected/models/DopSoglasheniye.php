...
class DopSoglasheniye extends CActiveRecord
{
    ...
    public function CreateEtapRabot()
    {
        /* $object_Rabot=new ObjectRabot();
         $object_Rabot->id_avtor=$this->id_author;
         $object_Rabot->id_dogovor=$this->id_dogovor;
         $object_Rabot->id_rayon=1;
         $object_Rabot->comment="Тестовый адрес, который вводится автоматически! Можно его править!";
         $object_Rabot->save(false);*/
        $object_Rabot = ObjectRabot::model()->findAllByAttributes(array('id_dogovor' => $this->id_dogovor));


        if ($object_Rabot == null) {
            $object_Rabot = new ObjectRabot();
            $object_Rabot->id_avtor = $this->author_id;
            $object_Rabot->id_dogovor = $this->id_dogovor;
            $object_Rabot->id_rayon = 1;
            $object_Rabot->comment = "Тестовый адрес, который вводится автоматически! Можно его править!";
            $object_Rabot->save(false);
            ...
            foreach ($object_Rabot as $objects=>$object)
            {
                $etap_rabot = new EtapRabotPoObjectu();
                $etap_rabot->id_objecta = $object['id'];
                $etap_rabot->data_nachala_rabot = $this->data_vneseniya;
                $etap_rabot->id_spr_etap_rabot = 56;
                $etap_rabot->comment = $this->comment;
                $etap_rabot->data_okonchaniya_rabot = $this->data_vneseniya;
                $etap_rabot->id_otvetstvennogo =$this->author_id? $this->author_id: Yii::app()->user->id;
...
// выбор ставки по договору исполнителя
 ...
 nds*/
    ...
    private function NumberGenerator()
    {
        ...
        $new_number =/* $model_dogovor->dogovor_number .*/
            (string)$index_number;
        return $new_number;
