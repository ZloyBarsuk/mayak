            ...
            if (isset($_POST['object_id'])) {
                ...
                if (isset($_POST['SoprovoditelniyList'])) {
                    ...
                    if (isset($_POST['select_all']) && $_POST['select_all'] === 'no') {
                        ...
                        if ($model->save()) {
                            //   $model->getIsNewRecord() ? $etap_rabot = new EtapRabotPoObjectu() : $etap_rabot = EtapRabotPoObjectu::model()->findByAttributes(array('id_objecta' => $model->id_objecta));



                            if ($model->id_polevik_1 || $model->id_polevik_2) {
                                $etap_rabot = new EtapRabotPoObjectu();
                                $etap_rabot->id_objecta = $model->id_objecta;
                                $etap_rabot->data_nachala_rabot = $model->data_vidachi_pole;
                                $etap_rabot->id_spr_etap_rabot = 121;
                                $etap_rabot->data_okonchaniya_rabot = $model->data_sdachi_pole;
                                $etap_rabot->id_otvetstvennogo = $model->id_proveril_pole? $model->id_proveril_pole : Yii::app()->user->id;
                                   $etap_rabot->save(false);
                            } if ($model->id_kameralka) {
                                $etap_rabot = new EtapRabotPoObjectu();
                                $etap_rabot->id_objecta = $model->id_objecta;
                                $etap_rabot->data_nachala_rabot = $model->data_vidachi_kameralka ? $model->data_vidachi_kameralka : Yii::app()->user->id;
                                $etap_rabot->id_spr_etap_rabot = 122;
                                $etap_rabot->data_okonchaniya_rabot = $model->data_sdachii_kameralka;
                                $etap_rabot->id_otvetstvennogo = $model->id_proveril_kameralka ? $model->id_proveril_kameralka : Yii::app()->user->id;
                                   $etap_rabot->save(false);
                            } if ($model->id_mejevoy) {
                                $etap_rabot = new EtapRabotPoObjectu();
                                $etap_rabot->id_objecta = $model->id_objecta;
                                $etap_rabot->data_nachala_rabot = $model->data_sdachi_mejevoy;
                                $etap_rabot->id_spr_etap_rabot = 123;
                                $etap_rabot->data_okonchaniya_rabot = $model->data_sdachi_mejevoy;
                                $etap_rabot->id_otvetstvennogo = $model->id_proveril_mejevoy ? $model->id_proveril_mejevoy : Yii::app()->user->id;
                                    $etap_rabot->save(false);
                            } else {
