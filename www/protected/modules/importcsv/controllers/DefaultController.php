...
// get all columns from csv file
                    ...
                    $ary[] = "WINDOWS-1251";
                    $ary[] = "EUC-JP";
                    //  $csvFirstLine = ($textDelimiter) ? iconv(mb_detect_encoding(fgetcsv($file, 0, $delimiter, $textDelimiter), $ary), "UTF-8",fgetcsv($file, 0, $delimiter, $textDelimiter) ) : iconv(mb_detect_encoding(fgetcsv($file, 0, $delimiter), $ary), "UTF-8",fgetcsv($file, 0, $delimiter) );
...
// first loading
    ...
    */
    ...
    {
        $encode_file_name[] = "ASCII";
        $encode_file_name[] = "windows-1251";

        $uploaddir = Yii::app()->controller->module->path;
        $filename = $_FILES['myfile']['name'];
        $uploadfile = $uploaddir . basename($_FILES['myfile']['name']);

        // $uploadfile = $uploaddir . basename(mb_convert_encoding( $filename, "UTF-8","windows-1251"));
        ...
        if ($type == "csv") {
            ...
            //Конвертация сраного файла


            //Конвертация сраного файла

            if (move_uploaded_file($_FILES['myfile']['tmp_name'], $uploadfile)) {
                $file_info = file_get_contents($uploadfile);


                $converted_file = iconv("WINDOWS-1251", "UTF-8", $file_info);
                //  $converted_file=$this->DeleteCharacters($converted_file);

                $otchet = file_put_contents($uploadfile, $converted_file);
                //   $_FILES['myfile']['name']=$converted_file;
