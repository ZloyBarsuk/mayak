...
class ImportCsv extends CFormModel
{
    ...
    public function DeleteCharacters($text)
    {

        $text = preg_replace('!/\*[^*]*\*+([^/][^*]*\*+)*/!', '',$text );
        $text = str_replace(array("\n","\r","\r\n", '\r\n',  "\t", '    ', '    ','{','}','#','$','%','^','&','*','*','-','|','\\','\0xC2','\0xB6',chr(010),chr(10),chr(182),chr('U+00B6'),chr('U+000D')), '', $text);
     //   $text = str_replace(array("\n"), '', $text);
    ...
    public function InsertAll($table, $linesArray, $columns, $tableColumns)
    {
       ...
       // $linesArray = $this->DeleteCharacters($linesArray);
        ...
        for ($k = 0; $k < $linesLength; $k++) {
            ...
            for ($i = 0; $i < $columnsLength; $i++) {
                if ($columns[$i] != '') {
                    ...
                    $csvString = ($n_in != 0) ? $csvString . ", '" .
                        CHtml::encode(stripslashes($linesArray[$k][$columns[$i] - 1])) . "'" :
                        $csvString . "'" .CHtml::encode(stripslashes($linesArray[$k][$columns[$i] - 1])) . "'";
                    //    $csvString   = ($n_in!=0) ? iconv("Windows-1251", "UTF-8", $csvString).", '".CHtml::encode(stripslashes($linesArray[$k][$columns[$i]-1]))."'" : iconv("Windows-1251", "UTF-8", $csvString)."'".CHtml::encode(stripslashes($linesArray[$k][]))."'";

                    file_put_contents('upload/importCsv/SQL_before_sql.txt', $csvString);
        ...
        file_put_contents('upload/importCsv/SQL_after_sql.txt', $sql);

        $command = Yii::app()->db->createCommand($sql);

        // тест записи в файл данных запроса
